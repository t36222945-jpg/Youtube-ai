...
YouTube Trending Opportunities Analyzer
"""
...
def identify_rising_niches(youtube, category_ids, num_niches=5):
    ...
    sorted_niches = sorted(niche_data.items(), key=lambda x: x[1]['growth_rate'], reverse=True)
...
def generate_title_with_ai(openai_key, niche, trending_titles):
    ...
    try:
        ...
        prompt = f"""أنت متخصص في إنشاء عناوين YouTube جاذبة.
        النيتش: {niche}
...
def generate_script_outline(openai_key, title, niche):
    ...
    try:
        openai.api_key = openai_key
        prompt = f"""أنت كاتب سيناريو YouTube متخصص.
        العنوان: {title}
        النيتش: {niche}
        أنشئ خطة فيديو: 1. المقدمة 2. المشكلة 3. الحل 4. الأمثلة 5. الخاتمة"""
        response = openai.ChatCompletion.create(
            model="gpt-3.5-turbo",
            messages=[{"role": "user", "content": prompt}],
            max_tokens=400,
...
if explore_button:
    ...
    if not opportunities_df.empty:
        ...
        fig = px.bar(opportunities_df.head(10), x='title', y='score', color='score', title='أفضل 10 فرص', color_continuous_scale='Viridis')
